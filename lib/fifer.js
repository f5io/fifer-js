(function(w,d) {
	...
	_.fallback = function() {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState === 4 && xhr.status === 404) {
				throw new Error('[Fifer] Fifer was unable to load the swf for the flash fallback, you can customise where it is using Fifer({ swf : *_file_path_* })');
	...
	fF.play = function($name, $loop) {
		...
		var p = _scope.playAudio($name, $loop);
		if (p) {
